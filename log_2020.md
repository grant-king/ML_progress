...
After running around in circles for a while, I finally figured out why media files weren't appearing on the site. I was editing the apache configuraiton with nano, and had made an error in one of the file alias rules set there. In trying to find the fix, I also went through the process to get the custom domain working with SSL. Finally, there were a few more external things to do to get SMTP working. With all of these in place, the site infrastructure is now fully functional. It is up, but I won't link to it from my various profile pages until I can add enough projects.
